...
Um exemplo de interação que vamos ter com R pode ser feituo com o comando R.version no terminal:

```{r}
R.version
```

O comando R.version mostra uma série de informações sobre a versão do R instalada no computador. Para terminar a execução do R basta executar o  comando q(). Segue outros comandos interessantes:
...
Quando executamos o R, somente algumas funções estão disponı́veis de imediato. Essas são as funções incluı́das nas packages incluídas e carregadas automaticamente. Para instalar uma nova package podemos usar a função install.packages que tem como argumento o nome da package a ser instalada:
...
O objeto mais básico do R para guardar dados é o vetor. Um vetor é uma estrutura de dados que permite armazenar um conjunto de valores do mesmo tipo (por exemplo números) sob um mesmo nome.
...
Os fatores proporcionam uma forma fácil e compacta de lidar com dados categóricos. Este tipo de dado pode ser visto como variáveis que podem tomar como valores possı́veis um conjunto finito derótulos  (por exemplo uma variável que
...
Podemos fazer um scterplot dessas informações:
