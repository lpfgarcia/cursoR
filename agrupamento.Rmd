...
Conseguimso descobrir qual o número de grupos ideal? Uma alternativa é utilizar uma medida de validação de agrupamento como a silhueta. 

![Formulação matemática da Silhueta](figures/silhueta.png)

Para isso precisamos carregar a biblioteca cluster.
...
Ou usar a média para avaliar se o agrupamento realizado pelo kMeans é bom.

```{r}
mean(sil[,3])
```

Podemos tentar encontrar o valor ótimo de grupos baseado na medida silhueta.
...
Também usando a biblioteca plotly.
...
Um experimento interessante é a tarefa de comprimir áudio utilizando o algoritmo de agrupamento kMeans. Para a compressão, o número de sinais de áudio precisa ser reduzido. A redução é feita através do agrupamento dos sinais mais frequentes. Para isso vamos precisar das bibliotecas audio e seewave.
...
Vamos baixar uma música da internet com permissão de reprodução pelo pixabay (https://pixabay.com/music/). Essa música tem o formato MP3.
...
Aplicando o kMeans para 10 grupos.
...
Adicionando os grupos encontrados no data.
