...
Uma função relacionada com a instrução if é a função ifelse. Esta função permite trabalhar com vectores de condições boleanas:
...
A instrução for permite controlar o número de vezes que um bloco de código será executado através de uma variável de controle. A sua sintaxe dela é:
...
Uma dessas funções, a mclapply permite a execução de blocos de código como a a lapply mas de forma paralela. Para utiliza-la precisamos carrecar a bibliteca parallel:
...
Os arquivos de texto são uma das formas mais comuns de armazenar dados. O formato de aruqivo mais comum é o CSV mas diversos outros tipos e formatos de arquivos são aceitos. Dentre eles temos o xls e o arff. Primeiro vamos criar uma arquivo CSV:
...
require('plot3D')
scatter3D(x=data[,2], y=data[,3], z=data[,4], colvar=as.integer(iris$Species), colkey=FALSE)
```
